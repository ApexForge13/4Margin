...
export interface DecoderPdfData {
  ...
  // Branding (optional — defaults to unbranded)
...
export function generateDecoderPdf(data: DecoderPdfData): ArrayBuffer {
  ...
  const headerTitle =
    brand === "decodecoverage"
      ? "DECODECOVERAGE POLICY ANALYSIS"
      : brand === "4margin"
        ? "4MARGIN POLICY ANALYSIS REPORT"
        : "POLICY ANALYSIS REPORT";

  const headerBg: [number, number, number] =
    brand === "decodecoverage"
      ? [17, 94, 89] // Teal-800 (DC brand)
      : brand === "4margin"
        ? [30, 58, 138] // Blue-900 (4M brand)
        : C.text;

  const footerLabel =
    brand === "decodecoverage"
      ? "DecodeCoverage  |  Powered by 4Margin"
      : brand === "4margin"
        ? "4Margin  |  Policy Intelligence for Contractors"
        : "Policy Analysis Report";

  const disclaimerText =
    brand === "decodecoverage"
      ? "This analysis is generated by DecodeCoverage for educational and informational purposes only. " +
        ...
        "DecodeCoverage is powered by 4Margin."
      : brand === "4margin"
        ? "This analysis is generated by 4Margin for professional reference only. " +
          "It does not constitute legal or insurance advice. Always verify policy provisions " +
          "with the carrier and consult licensed professionals for binding interpretations. " +
          "Results may not reflect all policy provisions, especially if only a declarations page was provided."
  ...
  setColor(C.white);
  doc.text(headerTitle, margin, 36);
  ...
  if (data.summaryForContractor) {
    ...
    doc.setFont("helvetica", "bold");
    setColor(C.text);
    doc.text(brand === "decodecoverage" ? "What This Means For You" : "Summary", margin + 12, y);
    y += 10;
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    setColor(C.textLight);
    doc.text(
      `${footerLabel}  |  ${data.generatedDate}`,
      margin,
      pageHeight - 14
    );
